...
class Database
{
    ...
    private function __construct()
    {
        try {
            ...
            $this->crearTablaProductos(); // Crear tabla Productos si no existe
    ...
    private function crearTablaProductos()
    {
        $this->pdo->exec("
    ...
    private function insertarProductosDeEjemplo()
    {
        ...
        if ($check == 0) {
            $categorias = ['Electrónica', 'Computadoras', 'Accesorios', 'Wearables', 'Fotografía', 'Monitores', 'Tabletas', 'Periféricos'];
            for ($i = 1; $i <= 10; $i++) {
                ...
                $this->pdo->exec("INSERT INTO productos (nombre, categoria, precio, stock) VALUES ('$nombre', '$categoria', $precio, $stock)");
            }
        }
    }
    ...
    private function insertarUsuarios()
    {
        ...
        if ($check == 0) {
            ...
            $passwordAdrian = password_hash('adrian', PASSWORD_DEFAULT);
            ...
            $this->pdo->exec("INSERT INTO usuarios (nombre, apellido, email, usuario, password)
                    VALUES ('adrian', 'garcia', 'adriandev@example.com', 'adrian', '$passwordAdrian')");
        }
    }
}
